...
- GrowthUIViewBridge：挂在成长面板桥接物体，例如 UIRoot/UIBridges/GrowthUIViewBridge。
- GrowthToolkitReceiver：挂在 UIRoot/UIBridges/GrowthToolkitReceiver，并拖到 GrowthUIViewBridge.Growth UI Receiver Provider。
- GrowthToolkitBindingProvider：挂在 UIRoot/UIToolkitRoot/BindingProviders/GrowthBindingProvider，并拖到 UIToolkitViewFactory.Binding Provider Behaviours。

默认 ViewId / BindingProviderId：GrowthPanel。

UXML 建议包含：TitleText、StatusText、ListRoot、DetailText、ResultText、EmptyRoot。Binding 会显示技艺名、等级、经验进度、分类、是否满级、是否可继承和缺失定义提示。

## 配置资产粒度基准

NiumaGrowth 按“一个成长项一个资产”管理。

- `GrowthSkillDefinition`：一种成长项、熟练度或技艺一个资产，例如剑术熟练、采药熟练、锻造熟练、社交等级。
- 角色当前经验、等级、是否继承不写进资产，而是运行时数据和存档。

同一个成长项可以被多个角色共享；不要为每个角色复制一份相同成长定义。
